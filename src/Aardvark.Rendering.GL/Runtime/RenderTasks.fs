...
module YetAnotherRenderTaskImpl =
    ...
    type AdaptiveGLVMFragment(obj : PreparedMultiRenderObject, adaptiveCode : IAdaptiveCode<Instruction>) =
        ...
        override x.InputChanged (t : Transaction, o : IAdaptiveObject) =
                    ...
                    let task = 
                        match pass.Order with
                            | RenderPassOrder.Arbitrary ->
                                new StaticOrderRenderTask(ctx, fboSignature, rtLock, config) :> AbstractRenderTask

                            | order ->
                                new CameraSortedRenderTask(order, ctx, fboSignature, rtLock, config) :> AbstractRenderTask
