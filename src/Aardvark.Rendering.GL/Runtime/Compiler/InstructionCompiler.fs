    ...
    let draw (program : Program) (indexArray : IMod<System.Array>) (call : IMod<DrawCallInfo[]>) (mode : IMod<IndexedGeometryMode>) (isActive : IMod<bool>) =
        ...
        let instructions =
            adaptive {
                ...
                let! (calls, isActive) = call, isActive

                let final =
                    calls |> Array.toList |> List.map (fun call ->
                        let faceVertexCount =
        ...
        instructions |> Mod.map (fun (mode,i) ->
            if hasTess then
                let size = patchSize mode
                let pp = Instruction.PatchParameter (int OpenTK.Graphics.OpenGL4.PatchParameterInt.PatchVertices) size
                pp::i
            else
                i
        )
